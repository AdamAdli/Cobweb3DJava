...
public class GLTest implements GLEventListener {
    ...
    protected String loadStringFileFromCurrentPackage(String fileName){
        InputStream stream = GLTest.class.getResourceAsStream(fileName);
    ...
    int newProgram(GL3 gl) {
        ...
        /*
           int v = this.newShaderFromCurrentClass2(gl, "/vertex.shader", ShaderType.VertexShader);
        int f = this.newShaderFromCurrentClass2(gl, "/fragment.shader", ShaderType.FragmentShader);
         */
        int v = this.newShaderFromCurrentClass2(gl, "#version 150\n" +
                "\n" +
        ...
        int f = this.newShaderFromCurrentClass2(gl, "#version 150\n" +
                "\n" +
                "in vec3 Color;\n" +
                "out vec4 outColor;\n" +
                "\n" +
    ...
    int newShaderFromCurrentClass2(GL3 gl, String shader, ShaderType type){
        // load the source
        String shaderSource = shader;
        // define the shaper type from the enum
        int shaderType = type==ShaderType.VertexShader?GL3.GL_VERTEX_SHADER:GL3.GL_FRAGMENT_SHADER;
        // create the shader id
        int id = gl.glCreateShader(shaderType);
