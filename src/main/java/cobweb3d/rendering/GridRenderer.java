...
public class GridRenderer {
    ...
    private Vector3Int[] mOuterLineVertexPositions;
    private Vector3Int[] mInnerLineVertexPositions;
    ...
    private void generateGeometry(Environment environment) {
        gridWidth = environment.width;
        gridHeight = environment.height;
        gridDepth = environment.depth;
        ...
        ArrayList<Vector3Int> mInnerVertexList = new ArrayList<>(
        ...
        ArrayList<Vector3Int> mOuterVertexList = new ArrayList<>();


        for (int x = 0; x < gridWidth + 1; x++) {
            for (int z = 0; z < gridDepth + 1; z++) {
                if (x == 0 || x == gridWidth || z == 0 || z == gridDepth) {
                    mOuterVertexList.add(new Vector3Int(x, 0, z));
                    mOuterVertexList.add(new Vector3Int(x, gridHeight, z));
                } else {
                    mInnerVertexList.add(new Vector3Int(x, 0, z));
                    mInnerVertexList.add(new Vector3Int(x, gridHeight, z));
                }
            }
        }
        for (int y = 0; y < gridHeight + 1; y++) {
            for (int z = 0; z < gridDepth + 1; z++) {
                if (y == 0 || y == gridHeight || z == 0 || z == gridDepth) {
                    mOuterVertexList.add(new Vector3Int(0, y, z));
                    mOuterVertexList.add(new Vector3Int(gridWidth, y, z));
                } else {
                    mInnerVertexList.add(new Vector3Int(0, y, z));
                    mInnerVertexList.add(new Vector3Int(gridWidth, y, z));
                }
            }
        }
        ...
        for (int x = 0; x < gridWidth + 1; x++) {
            for (int y = 0; y < gridHeight + 1; y++) {
                if (y == 0 || y == gridHeight || x == 0 || x == gridWidth) {
                    mOuterVertexList.add(new Vector3Int(x, y, 0));
                    mOuterVertexList.add(new Vector3Int(x, y, gridDepth));
                } else {
                    mInnerVertexList.add(new Vector3Int(x, y, 0));
                    mInnerVertexList.add(new Vector3Int(x, y, gridDepth));
                }
            }
        }

        mOuterVertexList.toArray(mOuterLineVertexPositions = new Vector3Int[mOuterVertexList.size()]);
        mInnerVertexList.toArray(mInnerLineVertexPositions = new Vector3Int[mInnerVertexList.size()]);
    }

    public void draw(Environment environment, GL2 gl) {
        if (environment.width != gridWidth
                || environment.height != gridHeight
                || environment.depth != gridDepth)
            generateGeometry(environment);

        gl.glBegin(GL.GL_LINES);
        gl.glColor3b((byte)101, (byte)101, (byte)101);
        for (Vector3Int position : mOuterLineVertexPositions) {
            gl.glVertex3f(position.x, position.y, position.z);
        }
        if (drawInnerGrid) {
            for (Vector3Int position : mInnerLineVertexPositions) {
                gl.glVertex3f(position.x, position.y, position.z);
            }
        }
