...
import cobweb3d.model.location.Direction;
import cobweb3d.model.location.Location;
import cobweb3d.model.location.LocationDirection;
import org.cobweb.cobweb2.core.location.Direction;
import org.cobweb.cobweb2.core.location.Location;
import org.cobweb.cobweb2.core.location.LocationDirection;
...
public class Topology {
	...
	public LocationDirection getAdjacent(LocationDirection location) {
		...
		if (wrap) {
			x = (x + width) % width; //(10 + 50) % 50 = 10 No idea what this code snippet does
            z = (z + depth) % depth;
			boolean flip = false;
			if (y < 0) {
				y = -y - 1;
			...
			if (flip) {
				x = (x + width / 2) % width; //(25 + 50/2) % 50 = 0
                z = (z + depth / 2) % depth;
	...
	private List<Location> getWrapVirtualLocations(Location l) {
		...
		if (wrap) {
			// wrap left
			result.add(new Location(l.x - width, l.y, l.z));
			// wrap right
			result.add(new Location(l.x + width, l.y, l.z));
